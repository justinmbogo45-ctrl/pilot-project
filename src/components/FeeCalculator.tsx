...
export const FeeCalculator:React.FC<FeeCalculatorProps>=({firms,initialFirm,initialPlan})=>{
  ...
  return <div className="max-w-5xl mx-auto p-6 space-y-6"><h1 className="text-2xl font-bold">Challenge fee & payout calculator</h1><p className="text-sm text-[#747976]">Uses the selected plan’s source currency. Enter missing values to calculate an estimate. Your assumptions do not change the catalog.</p>
    {!plan?<p>No account plans available yet.</p>:<>
      <div className="grid md:grid-cols-2 gap-4"><label className="text-sm">Firm<select aria-label="Calculator firm" className="block mt-2 w-full bg-[#1d1f1e] rounded p-3" value={firm!.id} onChange={e=>{setFirmId(e.target.value);selectPlan('');}}>{firms.filter(f=>f.plans.length).map(f=><option value={f.id} key={f.id}>{f.name}</option>)}</select></label><label className="text-sm">Account plan<select aria-label="Calculator plan" className="block mt-2 w-full bg-[#1d1f1e] rounded p-3" value={plan.id} onChange={e=>selectPlan(e.target.value)}>{firm!.plans.map(p=><option key={p.id} value={p.id}>{p.label} — {planPrice(p)}</option>)}</select></label></div>
      <div className="grid sm:grid-cols-2 gap-4">{[['size','Account size'],['fee','Challenge fee'],['drawdown','Maximum drawdown amount'],['split','Profit split (%)']].map(([key,label])=><label className="text-sm text-[#9a9e9b]" key={key}>{label}<input className="block mt-2 bg-[#1d1f1e] border border-[#333633] rounded p-3 w-full" type="number" min="0" step="any" placeholder="Not provided — enter an assumption" value={values[key]} onChange={e=>setOverrides({...overrides,[key]:e.target.value})}/></label>)}</div>
      <label className="block text-sm">Expected profit (%)<input className="ml-3 p-2 rounded bg-[#1d1f1e] w-24" type="number" min="0" step="any" value={profit} onChange={e=>setProfit(e.target.value)}/></label>
      ...
      <div className="grid sm:grid-cols-3 gap-4">{[['Estimated trader payout',money(net)],['Net after challenge fee',money(net!=null?net-fee!:null)],['Fee per 1,000 of drawdown',money(fee!=null&&drawdown!=null&&drawdown>0?fee/drawdown*1000:null)]].map(([label,value])=><div className="bg-[#1a1c1b] rounded-xl border border-[#2b2e2c] p-5" key={label}><p className="text-xs text-[#747976]">{label}</p><p className="mt-2 text-[#3ecf8e] font-bold">{value}</p></div>)}</div>
    </>}
  </div>;
};
