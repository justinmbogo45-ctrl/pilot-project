...
export const ComparisonMatrixModal:React.FC<ComparisonMatrixModalProps> = ({items,onRemoveItem,onClearAll,onChangePlan,onClose})=>{
  ...
  return <div role="dialog" aria-modal="true" aria-label="Firm comparison" className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={onClose}><div className="bg-[#0e1626] border border-slate-700 rounded-2xl p-5 max-w-6xl w-full max-h-[92vh] overflow-auto" onClick={e=>e.stopPropagation()}>
    ...
    <p className="text-xs text-slate-400 mb-4">Source prices retain their original currencies. Rules can vary by account and phase; open firm details for the full notes.</p>
    <table className="w-full text-sm text-left"><thead><tr><th className="p-3">Metric</th>{items.map(i=><th className="p-3 min-w-48" key={i.firm.id}><div className="flex justify-between gap-2">{i.firm.name}<button aria-label={`Remove ${i.firm.name}`} onClick={()=>onRemoveItem(i.firm.id)}>✕</button></div><select className="mt-2 w-full max-w-64 bg-slate-900 rounded text-xs p-2" value={i.plan.id} onChange={e=>{const p=i.firm.plans.find(p=>p.id===e.target.value);if(p)onChangePlan(i.firm.id,p);}}>{i.firm.plans.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}</select></th>)}</tr></thead>
    <tbody>{metrics.map(([label,get])=><tr className="border-t border-slate-800" key={label}><th className="p-3 text-slate-400 font-normal">{label}</th>{items.map(i=><td className="p-3" key={i.firm.id}>{displayValue(get(i))}</td>)}</tr>)}</tbody></table>
  </div></div>;
};
