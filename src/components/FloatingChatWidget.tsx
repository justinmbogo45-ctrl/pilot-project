...
export const FloatingChatWidget: React.FC<FloatingChatWidgetProps> = ({ firms, onSelectFirm }) => {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-40">
      {isOpen ? (
        <div className="w-[340px] sm:w-[400px] h-[520px] bg-[#121528] border border-blue-500/50 rounded-2xl shadow-2xl shadow-blue-950/60 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200">
          
          {/* Header with Google Grounding Badge */}
          <div className="bg-gradient-to-r from-blue-700 via-indigo-700 to-purple-700 p-3.5 flex items-center justify-between text-white shadow-md">
          ...
          <div className="flex-1 p-3.5 overflow-y-auto space-y-3.5 text-xs">
                ...
                <div
                  className={`p-3 rounded-xl max-w-[88%] leading-relaxed ${
                    m.sender === 'user'
                      ...
                      : 'bg-[#1a1f38] text-slate-200 border border-slate-700/80 rounded-bl-none shadow-sm'
                  }`}
                >
                      ...
                      <div className="flex flex-wrap gap-1">
                        {m.sources.slice(0, 3).map((s, sIdx) => (
                          <a
                            ...
                            className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-900 hover:bg-slate-800 border border-slate-700 text-[10px] text-blue-300 hover:text-white transition-colors"
                          >
            ...
            {loading && (
              <div className="flex items-start gap-2">
                <div className="p-3 rounded-xl bg-[#1a1f38] border border-blue-500/40 text-slate-300 text-xs flex items-center gap-2">
          ...
          <div className="px-3 py-1.5 bg-[#0e1120] border-t border-slate-800 flex items-center gap-1.5 overflow-x-auto text-[10px]">
            <button
              onClick={() => handleSend('Best active discounts right now')}
              className="px-2 py-1 rounded bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 whitespace-nowrap cursor-pointer transition-colors"
            >
              🔥 2026 Discounts
            </button>
            <button
          ...
          <form 
            ...
            className="p-2.5 bg-[#121528] border-t border-slate-800 flex items-center gap-2"
          >
            <input
              type="text"
              value={input}
              ...
              disabled={loading}
              className="flex-1 bg-slate-900 border border-slate-700 focus:border-blue-500 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none"
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="w-8 h-8 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white flex items-center justify-center transition-colors cursor-pointer"
            >
        ...
        <button
          ...
          className="w-13 h-13 rounded-full bg-gradient-to-tr from-blue-600 via-indigo-600 to-purple-600 text-white shadow-2xl shadow-blue-900/60 hover:scale-105 transition-all flex items-center justify-center cursor-pointer border border-blue-400/40 relative group"
